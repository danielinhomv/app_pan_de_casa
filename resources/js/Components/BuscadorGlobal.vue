...
<template>
    <div class="buscador-global relative flex items-center">
        <!-- Campo de búsqueda -->
        <div class="relative">
            <input ref="inputRef" v-model="query" type="text" placeholder="Buscar... (Ctrl+K)"
            ...
            <!-- Limpiar -->
            <button v-if="query" @click="query = ''; resultados = []; mostrar = false"
        ...
        <transition enter-active-class="transition ease-out duration-150" enter-from-class="opacity-0 translate-y-1"
            ...
            <div v-if="mostrar && (resultados.length > 0 || cargando)"
                ...
                <ul v-else class="divide-y divide-gray-50 dark:divide-gray-700 max-h-80 overflow-y-auto">
                    <li v-for="(r, i) in resultados" :key="i" @click="irA(r.url)"
                        ...
                        <div class="flex-1 min-w-0">
                            <p
                                class="text-sm font-medium text-gray-800 dark:text-gray-100 truncate group-hover:text-amber-600 transition-colors">
                                {{ r.titulo }}
                            </p>
                ...
                <div class="px-4 py-2 bg-gray-50 dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700">
                    <p class="text-xs text-gray-400 flex items-center gap-2">
                        <kbd
                            class="px-1.5 py-0.5 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded text-xs">↵</kbd>
                        para ir
                        <kbd
                            class="px-1.5 py-0.5 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded text-xs">Esc</kbd>
                        para cerrar
                    </p>
                </div>
            </div>
